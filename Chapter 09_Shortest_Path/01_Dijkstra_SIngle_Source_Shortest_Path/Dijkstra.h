...
public:
    ...
    Dijkstra(Graph &graph, int s):G(graph){
        ...
        marked = new bool[G.V()];
        for( int i = 0 ; i < G.V() ; i ++ ){
            distTo[i] = Weight();       // 由于不知道distTo中存储的类型是什么，所以使用模板类型Weight()的默认构造函数初始化
            marked[i] = false;          // 比如int(),double()也有这种初始化，只不过相应的都初始化成为了0
            from.push_back(NULL);
        }
    ...
    void shortestPath( int w, vector<Edge<Weight>> &vec ){
        ...
        while( e->v() != this->s ){                // 每次都是在找带权边的第一个节点，在已知第二个节点的情况下
            s.push(e);
            e = from[e->v()];
        }
        s.push(e);
