...
void __quickSort3Ways(T arr[], int l, int r){
    ...
    // 由于三路快排返回值最后是一个区间，因此就不写成partition函数的形式，直接分开区间就好
    ...
    int i = l+1;    // arr[lt+1...i) == v  三个区间开始时均为空这就有了一个正确的开始
    while( i < gt ){
        if( arr[i] < v ){
            swap( arr[i], arr[lt+1]);  //放入arr[l+1,lt]后面的一个位置lt+1
            i ++;
            lt ++;
        }
        else if( arr[i] > v ){
            swap( arr[i], arr[gt-1]);  //放入arr[gt,r]前面的一个位置 gt-1
            gt --;
        }
        else{ // arr[i] == v
            i ++;
