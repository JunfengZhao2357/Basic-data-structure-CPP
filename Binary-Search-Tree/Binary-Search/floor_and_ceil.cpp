...
int floor(T arr[], int n, T target){
    ...
    // 寻找比target小的最大索引
    int l = -1, r = n-1;             // 由于是在寻找floor，所以l的起始位置是数组首元素的前一个位置

    while( l < r ){

        // 使用向上取整避免死循环

        int mid = l + (r-l+1)/2;     // eg. l=4，r=5， mid=l+(r-l+1)/2,
                                     // 如果arr[mid]<target，l就会一直等于mid，陷入死循环

        if( arr[mid] >= target )     //arr[mid] >= target 就自动向左半区查找
            r = mid - 1;
        else
            l = mid;
    }
